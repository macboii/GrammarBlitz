...
| # | 작업 | 최소 검증 | 상태 |
|---|------|-----------|------|
| 5-1 | 콤보 카운터: 연속 성공(오답 격추 / 정답 접촉)마다 +1, FAIL 시 게임 종료 | HUD에 콤보 카운터 실시간 표시 | 대기 |
| 5-2 | 콤보 속도 보너스: 5콤보마다 fallSpeed 추가 부스트 + `🔥 xN` FloatingText | 5콤보 달성 시 체감 속도 증가 확인 | 대기 |
| 5-3 | Result Screen: Best Score 강조 표시 + `🔥 New Record!` 플래시 (갱신 시) + 최대 콤보 표시 | 신기록 갱신 시 플래시 애니메이션 동작 | 대기 |
