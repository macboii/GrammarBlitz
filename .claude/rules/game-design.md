...
스폰 직후 1.2초간 문장이 초록↔빨강으로 교차 점멸 (7Hz). 색상만으로 정답/오답을 판단할 수 없는 상태.

- 낙하 속도: 플래시 중 `fallSpeed * 0.5` (50% 감속 — 유저 판단 시간 확보)
- 라벨: `CORRECT`/`WRONG` 대신 `⚡ x2` 표시
- 플래시 중 올바른 행동 (오답 격추 / 정답 접촉) → **2배 점수** (`+4` / `+6`) + `⚡ x2` FloatingText
- 플래시 중 FAIL 케이스는 동일한 FAIL 규칙 적용 (추가 감점 없음)
- 1.2초 경과 후: 색상 고정(정답=초록, 오답=빨강), 라벨 표시, 정상 속도 및 점수로 복귀
