...
스폰 시 ~25% 확률로 **보너스(flash) 문장** 지정. 색상만으로 정답/오답을 판단할 수 없는 상태가 낙하 전체 구간 지속.

- `isBonus = true` 문장은 **낙하 시작부터 끝까지** 초록↔빨강 교차 점멸 (7Hz)
- 낙하 속도: `fallSpeed * 0.25` (75% 감속 — 유저 판단 시간 확보)
- 라벨: `CORRECT`/`WRONG` 대신 `⚡ x2` 표시
- 일반 문장은 플래시 없이 스폰 즉시 색상 고정

| 행동 | 오답 보너스 문장 | 정답 보너스 문장 |
|------|----------------|----------------|
| 총알로 맞추면 | `CorrectionBubble` 등장 + **+4** | FAIL |
| 플레이어가 먹으면 | `CorrectionBubble` 등장, **FAIL 없음**, 콤보 리셋 | **+6** + `⚡ x2` |
| 바닥 통과 | FAIL | FAIL |

## 오답 격추 정정 버블 (CorrectionBubble)

총알이 오답 문장에 맞으면 사라지지 않고 **정정 버블**이 위로 튀어나옴.

- `explanation` 필드의 `'wrong' → 'correct'` 패턴을 파싱해 문장 텍스트에 치환 적용
- 초록 배경 + `✓ FIXED` 라벨, 위로 날아오르며 1.8초간 페이드아웃
- 파싱 실패 시 원본 문장 그대로 표시 (silent fallback)
- 보너스 오답을 플레이어가 먹을 때도 동일하게 적용
